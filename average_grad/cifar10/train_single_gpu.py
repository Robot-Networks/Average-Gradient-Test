    ...
    for epoch_idx in range(1, args.train_epochs + 1):
        ...
        try:
            while True:
                ...
                inputs1, labels1 = next(data_iter)
                inputs1, labels1 = inputs1.cuda(), labels1.cuda()

                # zero out the gradients
                optimizer.zero_grad()

                # forward pass and compute first mini-batch gradients
                outputs1 = net(inputs1)
                loss1 = criterion(outputs1, labels1)
                loss1.backward()

                # get second mini-batch
                inputs2, labels2 = next(data_iter)
                inputs2, labels2 = inputs2.cuda(), labels2.cuda()

                # forward pass and compute second mini-batch gradients
                outputs2 = net(inputs2)
                loss2 = criterion(outputs2, labels2)
                loss2.backward() # accumulates gradients
